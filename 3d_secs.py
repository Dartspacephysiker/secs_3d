...
xg = read.grid(path)
extend=1
grid, grid_ev = helpers.make_csgrid(xg, height=height, crop_factor=0.4, #0.45
...
datadict = helpers.sample_at_alt(xg, dat, alt=alts_grid, altres=altres, 
                                 var=["je","jn","ju"], resfac=0.2) #0.5

#Prepare the sampled GEMINI data for inversion
d, lat, lon, alt = helpers.prepare_model_data(grid, datadict, alts_grid)

#Retrieve matrices to compute m from GEMINI data
Ge_cf, Gn_cf, Ge_df, Gn_df = helpers.get_SECS_J_G_matrices_3D(grid, alts_grid, 
                lat, lon, alt, interpolate=interpolate, 
                singularity_limit=singularity_limit)
S = helpers.get_jr_matrix(grid, alts_grid, lat, lon, alt, interpolate=interpolate)
O = np.zeros(S.shape)
...
lat, lon, alt = helpers.remove_outside(grid, alts_grid, lat_ev.flatten(), 
...
Ge_cf, Gn_cf, Ge_df, Gn_df = helpers.get_SECS_J_G_matrices_3D(grid, alts_grid, 
                                    lat, lon, alt, interpolate=interpolate, 
                                    singularity_limit=singularity_limit)
S = helpers.get_jr_matrix(grid, alts_grid, lat_ev, lon_ev, alt_ev, interpolate=interpolate)
O = np.zeros(S.shape)
Gcf = np.vstack((S, -Gn_cf, Ge_cf))
Gdf = np.vstack((O, -Gn_df, Ge_df))
G = np.hstack((Gcf, Gdf))
...
je_cs, jn_cs, ju_cs = helpers.grid_param_at_alt(datadict,grid,param='j')
...
import glob
files = glob.glob('./plots/tmp/snap_ju*.png')
files.sort()
gifname = './plots/tmp/ju_plot.gif'   
